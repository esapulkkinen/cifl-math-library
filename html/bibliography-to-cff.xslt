...
references:
  ...
  <xsl:choose><xsl:when test="field[@name='author']">
      authors:
      <xsl:for-each select="field[@name='author']/summary[@name='name']">        - name: "<xsl:value-of select="node()"/>"
      </xsl:for-each>
    </xsl:when>
  </xsl:choose>
    <xsl:for-each select="summary">
      summary: "<xsl:value-of select="node()"/>"
    </xsl:for-each>
  
    <xsl:for-each select="field"><xsl:sort select="attribute::name"/>
    <xsl:choose>
          <xsl:when test="@name='headline'">
      title: "<xsl:value-of select="node()"/>"
          </xsl:when>
          <xsl:when test="@name='publisher'">
            <xsl:for-each select="summary[@name='name']">
      publisher: "<xsl:value-of select="node()"/>"
            </xsl:for-each>
          </xsl:when>
          <xsl:when test="@name='isbn'">
      isbn: "<xsl:value-of select="node()"/>"
          </xsl:when>
          <xsl:when test="@name='numberOfPages'">
      pages: <xsl:value-of select="node()"/>
          </xsl:when>
          <xsl:when test="@name='license'">
      license: "<xsl:value-of select="node()"/>"
          </xsl:when>
          <xsl:when test="@name='url'">
      url: "<xsl:value-of select="node()"/>"
          </xsl:when>
          <xsl:when test="@name='genre'">
      genre: "<xsl:value-of select="node()"/>"
          </xsl:when>
    </xsl:choose>
