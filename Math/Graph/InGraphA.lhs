...
>instance (Monoid (m a a)) => MonoidArrow (InGraphA m a) (m a a) a where
>   monoidA m = proc i -> actA -< (i,m)

>actA :: InGraphA m a (a,m a a) a
>actA = InGraphA $ proc (a,m) -> do
>     g <- readState -< ()
>     ArrowTransformer.lift (uncurry (action g)) -<< (a,m)

>instance (Ord a,GraphMonoid m a) => GraphArrow (InGraphA m a) a where
...
>instance (Ord a, ReversibleGraphMonoid m a) => ReversibleGraphArrow (InGraphA m a) a where
>  ginverseA = inverseA
>  greversibleLinksA = reversibleLinksA

>elementsA :: (GraphMonoid m a) => InGraphA m a () (Set a)
>elementsA = InGraphA $ proc () -> do
>   g <- readState -< ()
>   returnA -< elements g

>verticesA :: (GraphMonoid m a, Ord a) => InGraphA m a () (Set a)
>verticesA = InGraphA $ proc () -> do
>   g <- readState -< ()
>   returnA -< Set.filter (\a -> action g a gdom == a) $ elements g

>edgesA :: (GraphMonoid m a, Ord a) => InGraphA m a () (Set a)
>edgesA = InGraphA $ proc () -> do
>   g <- readState -< ()
>   returnA -< Set.filter (\a -> action g a gdom /= a) $ elements g

>linksA :: (GraphMonoid m e, Ord e) => InGraphA m e () (Set (e,e,e))
>linksA = InGraphA $ proc () -> do
>   g <- readState -< ()
>   edges <- runInGraphA edgesA -< ()
>   returnA -< Set.map (\e -> (e,action g e gdom,action g e gcod)) edges

>edgesStartingFromA :: (GraphMonoid m a, Ord a) => InGraphA m a a (Set a)
>edgesStartingFromA = InGraphA $ proc x -> do
>   edges <- runInGraphA edgesA -< ()
>   g <- readState -< ()
>   returnA -< Set.filter (\a -> action g a gdom == x) edges

>edgesEndingToA :: (GraphMonoid m a, Ord a) => InGraphA m a a (Set a)
>edgesEndingToA = InGraphA $ proc x -> do
>   edges <- runInGraphA edgesA -< ()
>   g <- readState -< ()
>   returnA -< Set.filter (\a -> action g a gcod == x) edges

>reversibleLinksA :: (Ord a, ReversibleGraphMonoid m a) => InGraphA m a () (Set ((a,a),(a,a)))
...
>sourceA :: (GraphMonoid m a) => InGraphA m a a a
>sourceA = proc e -> actA -< (e,gdom)

>targetA :: (GraphMonoid m a) => InGraphA m a a a
>targetA = proc e -> actA -< (e,gcod)

>inverseA :: (ReversibleGraphMonoid m a) => InGraphA m a a a
>inverseA = proc e -> actA -< (e,gnot)

>edgeNodesA :: (GraphMonoid m a) => InGraphA m a a (a,a)
>edgeNodesA = proc e -> do
>   s <- sourceA -< e
>   t <- targetA -< e
>   returnA -< (s,t)

>isVertexA :: (Eq a, GraphMonoid m a) => InGraphA m a a Bool
>isVertexA = proc a -> do { a' <- sourceA -< a ; returnA -< a == a' }
  
>isLoopA :: (Eq a, GraphMonoid m a) => InGraphA m a a Bool
>isLoopA = proc a -> do a' <- sourceA -< a
>                       b' <- targetA -< a
>                       returnA -< a' == b'

>isOneLaneLoopA :: (Eq a, ReversibleGraphMonoid m a) => InGraphA m a a Bool
>isOneLaneLoopA = proc a -> do { a' <- inverseA -< a ; returnA -< a == a' }

>isEdgeA :: (Eq a,GraphMonoid m a) => InGraphA m a (a,a,a) Bool
